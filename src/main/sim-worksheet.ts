...
export async function buildSimWorksheetPdf(input: {
  patient: PatientRecord;
  course: TreatmentCourseRecord;
  visit: VisitNoteRecord;
}) {
  const pdfDoc = await PDFDocument.load(getWorksheetPdfBytes());
  ...
  fillSite(form, SITE_1_FIELDS, sites.find((site) => site.siteNumber === 1) ?? null, input.visit.structuredFields.additionalNotes, projectedFractions);
  fillSite(form, SITE_2_FIELDS, sites.find((site) => site.siteNumber === 2) ?? null, input.visit.structuredFields.additionalNotes, projectedFractions);
