...
export class DesktopPatientArchiveReaderService {
  async readArchive(archive: PatientArchiveIoHandle): Promise<PatientArchiveReadResult> {
    const issues: PatientArchiveValidationIssue[] = [];

    let zip: JSZip;
    try {
      zip = await JSZip.loadAsync(fs.readFileSync(archive.path));
    } catch (error) {
      issues.push({
        severity: "error",
    ...
    const fileEntries: ImportedArchiveFileEntry[] = await Promise.all(
      Object.values(zip.files).map(async (entry) => {
        ...
        return {
          packagePath: entry.name,
          isDirectory: false,
          // JSZip does not expose compressed size through its public typed API,
          // so we track the unpacked byte length for now and keep the field ready
          // for later format-specific enrichment.
